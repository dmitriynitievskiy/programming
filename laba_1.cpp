...
//функция нахождения минимального элеменат массива
...
//функция удаления элементов с четными числами после минимального 
	void func (int *array, int &size, int min, int min_pos) {
        ...
        for (int i=min_pos+1; i<size; i++) {
					   ...
					   while (array[i] != 0) {
						   num=array[i] % 10;         //выделяем цифру числа
						   array[i]=array[i] / 10;

						   if (num % 2 == 0) {         //проверяем является ли цифра четной
							   pr=true;
					   ...
					   if (!pr) array[i]=rem;        //если элемент массива не содержит четных цифр, оставляем данное число без изменений
                       pr=false;
				   }
		}
